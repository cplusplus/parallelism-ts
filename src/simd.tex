...
namespace std::experimental {
  inline namespace parallelism_v2 {
    ...
    int find_first_set(@\UNSP{T}@) noexcept;
    int find_last_set(@\UNSP{T}@) noexcept;
...
\pnum
Use of the \tcode{scalar} tag type requires data-parallel types to store a single element (i.e., \tcode{simd<T, simd_abi::scalar>::size()} returns 1). \begin{note}\tcode{scalar} is not an alias for \tcode{fixed_size<1>}.\end{note}
...
\begin{example}
  ...
  \begin{itemize}
    \item \tcode{compatible<T>} is an alias for \tcode{__simd128} for all vectorizable \tcode{T}, except \tcode{long double}, and
    \item \tcode{compatible<long double>} as an alias for \tcode{scalar}.
  \end{itemize}
\end{example}
...
\begin{itemize}
  ...
  \item \tcode{simd_abi::fixed_size<N>} is supported (see \tref{parallel.simd.abi}).
\end{itemize}
...
\begin{itemdescr}
\pnum
The type \tcode{is_abi_tag<T>} is a \tcode{UnaryTypeTrait} with a \tcode{BaseCharacteristic} of \tcode{true_type} if \tcode{T} is a standard or extended ABI tag, and \tcode{false_type} otherwise.
...
\begin{itemdescr}
\pnum
The type \tcode{is_simd<T>} is a \tcode{UnaryTypeTrait} with a \tcode{BaseCharacteristic} of \tcode{true_type} if \tcode{T} is a specialization of the \tcode{simd} class template, and \tcode{false_type} otherwise.
...
\begin{itemdescr}
\pnum
The type \tcode{is_simd_mask<T>} is a \tcode{UnaryTypeTrait} with a \tcode{BaseCharacteristic} of \tcode{true_type} if \tcode{T} is a specialization of the \tcode{simd_mask} class template, and \tcode{false_type} otherwise.
...
\begin{itemdescr}
\pnum
The type \tcode{is_simd_flag_type<class T>} is a \tcode{UnaryTypeTrait} with a \tcode{BaseCharacteristic} of \tcode{true_type} if \tcode{T} is one of
\begin{itemize}
...
\pnum
If \tcode{value} is present, the type \tcode{simd_size<T, Abi>} is a \tcode{BinaryTypeTrait} with a \tcode{BaseCharacteristic} of \tcode{integral_constant<size_t, N>} with \tcode{N} equal to the number of elements in a \tcode{simd<T, Abi>} object. \begin{note}If \tcode{simd<T, Abi>} is not supported for the currently targeted system, \tcode{simd_size<T, Abi>::value} produces the value \tcode{simd<T, Abi>::size()} would return if it were supported.\end{note}
...
\begin{itemdecl}
\tcode{template<class T, class U = typename T::value_type> struct memory_alignment { @\seebelow@ };}
\end{itemdecl}
\begin{itemdescr}
\pnum
...
\begin{itemdescr}
  \requires
  If the template parameter \tcode{Flags} is \tcode{vector_aligned_tag}, \tcode{mem} shall point to storage aligned by \tcode{memory_alignment_v<T, U>}. If the template parameter \tcode{Flags} is \tcode{overaligned_tag<N>}, \tcode{mem} shall point to storage aligned by \tcode{N}. If the template parameter \tcode{Flags} is \tcode{element_aligned_tag}, \tcode{mem} shall point to storage aligned by \tcode{alignof(U)}. If \tcode{M} is not \tcode{bool}, the largest \tcode{i} $\in$ \tcode{[0, M::size())} where \tcode{mask[i]} is \tcode{true} is less than the number of values pointed to by \tcode{mem}.

  \effects
  Copies the selected elements as if \tcode{mem[i] = static_cast<U>(data[i])} for all selected indices \tcode{i}.
...
\begin{itemdescr}
  ...
  \remarks
  Each of these operators shall not participate in overload resolution unless the return type of \tcode{data @ std::forward<U>(x)} is convertible to \tcode{T}. It is unspecified whether the binary operator, implied by the compound assignment operator, is executed on all elements or only on the selected elements.
\end{itemdescr}
...
\begin{itemdescr}
  \requires
  If the template parameter \tcode{Flags} is \tcode{vector_aligned_tag}, \tcode{mem} shall point to storage aligned by \tcode{memory_alignment_v<T, U>}. If the template parameter \tcode{Flags} is \tcode{overaligned_tag<N>}, \tcode{mem} shall point to storage aligned by \tcode{N}. If the template parameter \tcode{Flags} is \tcode{element_aligned_tag}, \tcode{mem} shall point to storage aligned by \tcode{alignof(U)}. If \tcode{is_simd_flag_type_v<U>} is \tcode{true}, for all selected indices \tcode{i}, \tcode{i} shall be less than the number of values pointed to by \tcode{mem}.

  \effects
  Replaces the selected elements as if \tcode{data[i] = static_cast<value_type>(mem[i])} for all selected indices \tcode{i}.
...
\begin{itemize}
  ...
  \item \tcode{Abi} is \tcode{simd_abi::fixed_size<N>}, with \tcode{N} is constrained as defined in \tref{parallel.simd.abi}.
\end{itemize}
...
\begin{itemdescr}
  \effects
  Replaces the referred to element in \tcode{simd} or \tcode{simd_mask} with \tcode{static_cast<value_type>(std::forward<U>(x))}.

  \returns
  A copy of \tcode{*this}.

  \remarks
  This function shall not participate in overload resolution unless \tcode{declval<value_type \&>() = std::forward>U>(x)} is well-formed.
\end{itemdescr}
...
\begin{itemdescr}
  \effects
  Applies the indicated compound operator to the referred to element in \tcode{simd} or \tcode{simd_mask} and \tcode{std::forward<U>(x)}.

  \returns
  A copy of \tcode{*this}.

  \remarks
  This function shall not participate in overload resolution unless \tcode{declval<value_type \&>() @= std::forward<U>(x)} (where \tcode{@=} denotes the indicated compound assignment operator) is well-formed.
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \throws
  Any exception thrown while converting the argument to \tcode{value_type}.

  \remarks
  Let \tcode{From} identify the type \tcode{remove_cv_t<remove_reference_t<U>>}. This constructor shall not participate in overload resolution unless:
...
\begin{itemdescr}
  \effects
  Constructs an object where the $i$-th element equals \tcode{static_cast<T>(x[i])} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  \effects
  Constructs an object where the $i$-th element is initialized to \tcode{gen(integral_constant<size_t, i>())}.

  \remarks
  This constructor shall not participate in overload resolution unless \tcode{simd(gen(integral_constant<size_t, i>()))} is well-formed for all \tcode{i} &#8714; \tcode{[0, size())}.

  \pnum
  The calls to \tcode{gen} are unsequenced with respect to each other. Vectorization-unsafe standard library functions may not be invoked by \tcode{gen} ([algorithms.parallel.exec]).
\end{itemdescr}
...
\begin{itemdescr}
  \requires
  If the template parameter \tcode{Flags} is \tcode{vector_aligned_tag}, \tcode{mem} shall point to storage aligend by \tcode{memory_alignment_v<simd, U>}. If the template parameter \tcode{Flags} is \tcode{overaligned_tag<N>}, \tcode{mem} shall point to storage aligned by \tcode{N}. If the template parameter \tcode{Flags} is \tcode{element_aligned_tag}, \tcode{mem} shall point to storage aligned by \tcode{alignof(U)}. \tcode{[mem, mem + size())} is a valid range.

  \effects
  Constructs an object where the $i$-th element is initialized to \tcode{static_cast<T>(mem[i])} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  \requires
  If the template parameter \tcode{Flags} is \tcode{vector_aligned_tag}, \tcode{mem} shall point to storage aligned by \tcode{memory_alignment_v<simd, U>}. If the template parameter \tcode{Flags} is \tcode{overaligned_tag<N>}, \tcode{mem} shall point to storage aligend by \tcode{N}. If the template parameter \tcode{Flags} is \tcode{element_aligned_tag}, \tcode{mem} shall point to storage aligned by \tcode{alignof(U)}. \tcode{[mem, mem + size())} is a valid range.

  \effects
  Replaces the elements of the \tcode{simd} object such that the $i$-th element is assigned with \tcode{static_cast<T>(mem[i])} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  ...
  \effects
  Copies all \tcode{simd} elements as if \tcode{mem[i] = static_cast<U>(operator[](i))} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  ...
  \returns
  A \tcode{reference} (see \tref{parallel.simd.reference}) referring to the $i$-th element.
...
\begin{itemdescr}
  \returns
  A \tcode{simd_mask} object with the $i$-th element set to \tcode{!operator[](i)} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  \returns
  A \tcode{simd} object where the $i$-th element is initialized to \tcode{-operator[](i)} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  \returns
  A \tcode{simd} object initialized with the results of the element-wise application of the indicated operator.
...
\begin{itemdescr}
  \returns
  A \tcode{simd} object where the $i$-th element is initialized to the result of applying the indicated operator to \tcode{v[i]} and \tcode{n} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  \returns
  A \tcode{simd_mask} object initialized with the results of the element-wise application of the indicated operator.
...
\begin{itemdescr}
  ...
  \returns
  \tcode{@\placeholdernc{GENERALIZED_SUM}@(binary_op, x.data[i], ...)} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  ...
  \returns
  If \tcode{none_of(x.mask)}, returns \tcode{identity_element}. Otherwise, returns \tcode{@\placeholdernc{GENERALIZED_SUM}@(binary_op, x.data[i], ...)} for all \tcode{i} &#8714; {<em>j</em> &#8714; &#8469;<sub>0</sub> &#8739; <em>j</em> < \tcode{M::size()} &#8896; \tcode{mask[}<em>j</em>\tcode{]} }.
...
\begin{itemdescr}
  \returns
  If \tcode{none_of(x.mask)}, returns \tcode{0}. Otherwise, returns \tcode{@\placeholdernc{GENERALIZED_SUM}@(binary_op, x.data[i], ...)} for all \tcode{i} &#8714; {<em>j</em> &#8714; &#8469;<sub>0</sub> &#8739; <em>j</em> < \tcode{M::size()} &#8896; \tcode{mask[}<em>j</em>\tcode{]} }.
...
\begin{itemdescr}
  \returns
  If \tcode{none_of(x.mask)}, returns \tcode{1}. Otherwise, returns \tcode{@\placeholdernc{GENERALIZED_SUM}@(binary_op, x.data[i], ...)} for all \tcode{i} &#8714; {<em>j</em> &#8714; &#8469;<sub>0</sub> &#8739; <em>j</em> < \tcode{M::size()} &#8896; \tcode{mask[}<em>j</em>\tcode{]} }.
...
\begin{itemdescr}
  ...
  \returns
  If \tcode{none_of(x.mask)}, returns \tcode{~V::value_type()}. Otherwise, returns \tcode{@\placeholdernc{GENERALIZED_SUM}@(binary_op, x.data[i], ...)} for all \tcode{i} &#8714; {<em>j</em> &#8714; &#8469;<sub>0</sub> &#8739; <em>j</em> < \tcode{M::size()} &#8896; \tcode{mask[}<em>j</em>\tcode{]} }.
...
\begin{itemdescr}
  \returns
  The value of an element \tcode{x[j]} for which \tcode{x[j] <= x[i]} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  \returns
  If \tcode{none_of(x.mask)}, the return value is \tcode{numeric_limits<V::value_type>::max()}. Otherwise, returns the value of an element \tcode{x.data[j]} for which \tcode{x.mask[j] == true} and \tcode{x.data[j] <= x.data[i]} for all \tcode{i} &#8714; {<em>j</em> &#8714; &#8469;<sub>0</sub> &#8739; <em>j</em> < \tcode{M::size()} &#8896; \tcode{mask[}<em>j</em>\tcode{]} }.
...
\begin{itemdescr}
  \returns
  If \tcode{none_of(x.mask)}, the return value is \tcode{numeric_limits<V::value_type>::lowest()}. Otherwise, returns the value of an element \tcode{x.data[j]} for which \tcode{x.mask[j] == true} and \tcode{x.data[j] >= x.data[i]} for all \tcode{i} &#8714; {<em>j</em> &#8714; &#8469;<sub>0</sub> &#8739; <em>j</em> < \tcode{M::size()} &#8896; \tcode{mask[}<em>j</em>\tcode{]} }.
...
\begin{itemdescr}
  \pnum
  Let \tcode{To} identify \tcode{T::value_type} if \tcode{is_simd_v<T>} is \tcode{true}, or \tcode{T} otherwise.

  \returns
  A \tcode{simd} object with the $i$-th element initialized to \tcode{static_cast<To>(x[i])} for all \tcode{i} &#8714; \tcode{[0, size())}.
  ...
  The return type is
  \begin{itemize}
    \item \tcode{T} if \tcode{is_simd_v<T>} is \tcode{true}, otherwise
    \item \tcode{simd<T, Abi>} is \tcode{U} is \tcode{T}, otherwise
    \item \tcode{simd<T, simd_abi::fixed_size<simd<U, Abi>::size()>>}
  \end{itemize}
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \begin{itemize}
    \item \tcode{T} if \tcode{is_simd_v<T>} is \tcode{true}, otherwise
    \item \tcode{simd<T, Abi>} if either \tcode{U} is \tcode{T} or \tcode{U} and \tcode{T} are integral types that only differ in signedness, otherwise
    \item \tcode{simd<T, simd_abi::fixed_size<simd<U, Abi>::size()>>}.
  \end{itemize}
\end{itemdescr}
...
\begin{itemdescr}
  \returns
  A data-parallel object with the $i$-th element initialized to \tcode{x[i]} for all \tcode{i} &#8714; \tcode{[0, size())}.
\end{itemdescr}
...
\begin{itemdescr}
  \returns
  A \tcode{tuple} of data-parallel objects with the $i$-th \tcode{simd}/\tcode{simd_mask} element of the $j$-th \tcode{tuple} element initialized to the value of the element \tcode{x} with index $i$ + sum of the first $j$ values in the \tcode{Sizes} pack.
...
\begin{itemdescr}
  \returns
  An \tcode{array} of data-parallel objects with the $i$-th \tcode{simd}/\tcode{simd_mask} element of the $j$-th \tcode{array} element initialized to the value of the element in \tcode{x} with index \tcode{$i$ + $j$ * V::size()}.
  ...
  \begin{itemize}
    ...
    \item for the overload with a \tcode{simd} parameter \tcode{is_simd_v<V>} is \tcode{true}, for the overload with a \tcode{simd_mask} parameter \tcode{is_simd_mask_v<V>} is \tcode{true}.
  \end{itemize}
\end{itemdescr}
...
\begin{itemdescr}
  \returns
  The result of the element-wise application of \tcode{std::min(a[i], b[i])} for all \tcode{i} &#8714; \tcode{[0, size())}.
\end{itemdescr}
...
\begin{itemdescr}
  \returns
  The result of the element-wise application of \tcode{std::max(a[i], b[i])} for all \tcode{i} &#8714; \tcode{[0, size())}.
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \begin{itemize}
    \item the result of element-wise application of \tcode{std::min(a[i], b[i])} for all \tcode{i} &#8714; \tcode{[0, size())} in the \tcode{first} member, and
...
\begin{itemdescr}
  ...
  \returns
  The result of element-wise application of \tcode{std::clamp(v[i], lo[i], hi[i])} for all \tcode{i} &#8714; \tcode{[0, size())}.
\end{itemdescr}
...
\pnum
Each function overload produced by the above rules applies the indicated \tcode{<cmath>} function element-wise. The results per element are not required to be bitwise equal to the application of the function which is overloaded for the element type.
...
\begin{itemdescr}
  \effects
  Constructs an object of type \tcode{simd_mask} where the $i$-th element equals \tcode{x[i]} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  \requires
  If the template parameter \tcode{Flags} is \tcode{vector_aligned_tag}, \tcode{mem} shall point to storage aligned by \tcode{memory_alignment_v<simd_mask>}. If the template parameter \tcode{Flags} is \tcode{overaligned_tag<N>}, \tcode{mem} shall point to storage aligned by \tcode{N}. If the template parameter \tcode{Flags} is \tcode{element_aligned_tag}, \tcode{mem} shall point to storage aligned by \tcode{alignof(U)}. \tcode{[mem, mem + size())} is a valid range.

  \effects
  Constructs an object where the $i$-th element is initialized to \tcode{mem[i]} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  ...
  \effects
  Replaces the elements of the \tcode{simd_mask} object such that the $i$-th element is replaced with \tcode{mem[i]} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  ...
  \effects
  Copies all \tcode{simd_mask} elements as if \tcode{mem[i] = operator[](i)} for all \tcode{i} &#8714; \tcode{[0, size())}.
...
\begin{itemdescr}
  \returns
  A \tcode{simd_mask} object initialized with the results of the element-wise appliation of the indicated operator.
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \returns
  The lowest element index \tcode{i} where \tcode{k[i]} is \tcode{true}.
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \returns
  The greatest element index \tcode{i} where \tcode{k[i]} is \tcode{true}.
\end{itemdescr}
...
\begin{itemdescr}
  ...
  \remarks
  The functions shall not participate in overload resolution unless the argument is of type \tcode{bool}.
\end{itemdescr}
