...
\newcommand\forallmaskedi{for all $i \in \{j \in \mathbb{N} | j < \tcode{M::size()} \wedge \tcode{mask[}j\tcode{]} \}$}
...
inline namespace parallelism_v2 {
  ...
  template<class T, class Abi> @\ednote{The template parameters are wrong. P0214R9 is correct.}@
...
\begin{itemdescr}
  \pnum\effects
  Replaces \tcode{data[i]} with \tcode{static_cast<T>(std\colcol{}forward<U>(x))[i]} for all selected indices \tcode{i}.
...
\begin{itemdescr}
  \pnum\effects
  Replaces \tcode{data[i]} with \tcode{static_cast<T>(data @ std\colcol{}forward<U>(x))[i]} (where \tcode{@} denotes the indicated operator) for all selected indices \tcode{i}.
...
\begin{itemdescr}
  \pnum\effects
  Constructs an object where the $i$-th element equals \tcode{static_cast<T>(x[i])} \foralli.
...
\begin{itemdescr}
  ...
  \pnum\effects
  Constructs an object where the $i$-th element is initialized to \tcode{static_cast<T>(mem[i])} \foralli.
...
\begin{itemdescr}
  ...
  \pnum\effects
  Replaces the elements of the \tcode{simd} object such that the $i$-th element is assigned with \tcode{static_cast<T>(mem[i])} \foralli.
...
\begin{itemdescr}
  \pnum\returns
  A \tcode{simd_mask} object with the $i$-th element set to \tcode{!operator[](i)} \foralli.
...
\begin{itemdescr}
  \pnum\returns
  A \tcode{simd} object where the $i$-th element is initialized to \tcode{-operator[](i)} \foralli.
...
\begin{itemdescr}
  \pnum\returns
  A \tcode{simd} object where the $i$-th element is initialized to the result of applying the indicated operator to \tcode{v[i]} and \tcode{n} \foralli.
...
\begin{itemdescr}
  ...
  \pnum\returns
  \tcode{\placeholdernc{GENERALIZED_SUM}(binary_op, x.data[i], ...)} \foralli.
...
\begin{itemdescr}
  ...
  \pnum\returns
  A \tcode{simd} object with the $i$-th element initialized to \tcode{static_cast<To>(x[i])} \foralli.
...
\begin{itemdescr}
  \pnum\returns
  A data-parallel object with the $i$-th element initialized to \tcode{x[i]} \foralli.
\end{itemdescr}
...
\begin{itemdescr}
  \pnum\effects
  Constructs an object of type \tcode{simd_mask} where the $i$-th element equals \tcode{x[i]} \foralli.
...
\begin{itemdescr}
  ...
  \pnum\effects
  Constructs an object where the $i$-th element is initialized to \tcode{mem[i]} \foralli.
...
\begin{itemdescr}
  ...
  \pnum\effects
  Replaces the elements of the \tcode{simd_mask} object such that the $i$-th element is replaced with \tcode{mem[i]} \foralli.
